...
public class M2_Boss : MonoBehaviour
{
    ...
    public void bossLeftRigt()
    {
        if (transform.position.y <= whenToStopY)
        {
            ...
            if (moveLeft == true)
            {
                ...
                if (transform.position.x <= rightpeak) { moveLeft = false; }
            }
            else
            {
                transform.Translate(-player_speed * Time.deltaTime, 0, 0);
                if (transform.position.x >= leftpeak) { moveLeft = true; }
            }
        }
    }
}
